...
- 索引其他规则
  ...
  - 聚集索引
    - 聚集索引是按照表中主键的顺序组织数据，每个表只能有一个聚集索引。在聚集索引中，数据在物理存储上是连续的，因此查询数据时速度较快。然而，插入数据时，因为需要找到合适的位置插入数据，速度可能会较慢
  - 非聚集索引
    - 非聚集索引是按照表中非主键列的顺序组织数据。在非聚集索引中，索引的叶子节点存储的是主键和索引列的数据，因此需要先通过索引查找到主键，再通过主键去查找想要的数据，即需要进行回表操作。非聚集索引也叫做二级索引，在建立的时候也未必是单列的，可以多个列来创建索引。
  - 回表
...
- MySQL死锁是指两个或多个事务在执行过程中，因争夺资源而造成的一种相互等待的现象，若无外力干涉它们都将无法继续执行
  - 竞争条件：多个事务同时对数据库中的同一数据进行操作，如多个用户同时对同一数据进行查询、修改等操作，导致数据竞争。
  - 事务锁：事务在执行过程中，可能会对数据进行加锁，以避免其他事务对数据进行修改。如果事务在执行过程中未释放锁，其他事务也无法获取到锁，从而产生死锁。
  - 行锁：MySQL中的行锁是用来控制对数据的并发访问的。如果多个事务同时对同一行数据进行操作，就可能导致行锁竞争，从而产生死锁
  - 回滚操作：当事务执行过程中发生错误或需要回滚操作时，MySQL需要先释放事务持有的所有锁，这可能会导致其他事务等待该事务释放锁，从而产生死锁
